...
!function() {
    function t(i) {
        ...
        this._canvas = i = "string" == typeof i ? document.getElementById(i) : i, this._ctx = i.getContext("2d"), 
        this._width = i.width, this._height = i.height, this._max = 1, this.clear();
    }
    t.prototype = {
        defaultRadius: 25,
        defaultGradient: {
        ...
        radius: function(t, i) {
            ...
            return a.width = a.height = 2 * e, s.shadowOffsetX = s.shadowOffsetY = 200, s.shadowBlur = i, 
            s.shadowColor = "black", s.beginPath(), s.arc(e - 200, e - 200, t, 0, 2 * Math.PI, !0), 
        ...
        gradient: function(t) {
            ...
            return a.fillStyle = s, a.fillRect(0, 0, 1, 256), this._grad = a.getImageData(0, 0, 1, 256).data, 
            this;
        },
        draw: function(t) {
            ...
            for (var a, s = 0, e = this._data.length; s < e; s++) a = this._data[s], i.globalAlpha = Math.max(a[2] / this._max, t || .05), 
        ...
        _colorize: function(t, i) {
            for (var a, s = 3, e = t.length; s < e; s += 4) (a = 4 * t[s]) && (t[s - 3] = i[a], 
    ...
    showHideMatching: function(t) {
        this._latlngs.each(function(i) {
            5 === i.length && (i[4] = i[3] === t);
        }), this.redraw();
    },
    ...
    redraw: function() {
        return this._heat && !this._frame && this._map && !this._map._animating && (this._frame = L.Util.requestAnimFrame(this._redraw, this)), 
        this;
    },
    onAdd: function(t) {
        this._map = t, this._canvas || this._initCanvas(), this.options.pane ? this.getPane().appendChild(this._canvas) : t._panes.overlayPane.appendChild(this._canvas), 
        t.on("moveend", this._reset, this), t.options.zoomAnimation && L.Browser.any3d && t.on("zoomanim", this._animateZoom, this), 
        this._reset();
    },
    onRemove: function(t) {
    ...
    _initCanvas: function() {
        ...
        L.DomUtil.addClass(t, "leaflet-zoom-" + (s ? "animated" : "hide")), this._heat = simpleheat(t), 
        this._updateOptions();
    },
    ...
    _reset: function() {
        ...
        this._heat._width !== i.x && (this._canvas.width = this._heat._width = i.x), this._heat._height !== i.y && (this._canvas.height = this._heat._height = i.y), 
        this._redraw();
    },
    _redraw: function() {
        if (this._map) {
            var t, i, a, s, e, n, h, o, r, d = [], _ = this._heat._r, l = this._map.getSize(), m = new L.Bounds(L.point([ -_, -_ ]), l.add([ _, _ ])), c = void 0 === this.options.max ? 1 : this.options.max, u = void 0 === this.options.maxZoom ? this._map.getMaxZoom() : this.options.maxZoom, f = 1 / Math.pow(2, Math.max(0, Math.min(u - this._map.getZoom(), 12))), p = _ / 2, g = [], v = this._map._getMapPanePos(), w = v.x % p, y = v.y % p;
            for (t = 0, i = this._latlngs.length; t < i; t++) {
                var x = this._latlngs[t];
                if ((5 !== x.length || !1 !== x[4]) && (a = this._map.latLngToContainerPoint(x), 
                m.contains(a))) {
                    e = Math.floor((a.x - w) / p) + 2, n = Math.floor((a.y - y) / p) + 2;
                    r = (void 0 !== x.alt ? x.alt : void 0 !== x[2] ? +x[2] : 1) * f, g[n] = g[n] || [], 
                    s = g[n][e], s ? (s[0] = (s[0] * s[2] + a.x * r) / (s[2] + r), s[1] = (s[1] * s[2] + a.y * r) / (s[2] + r), 
                    s[2] += r) : g[n][e] = [ a.x, a.y, r ];
                }
            }
            for (t = 0, i = g.length; t < i; t++) if (g[t]) for (h = 0, o = g[t].length; h < o; h++) (s = g[t][h]) && d.push([ Math.round(s[0]), Math.round(s[1]), Math.min(s[2], c) ]);
